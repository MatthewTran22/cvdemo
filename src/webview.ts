...
export function setupExpressRoutes(server: AppServer): void {
  ...
  app.get('/webview', (req: AuthenticatedRequest, res: any) => {
    if (req.authUserId) {
      ...
      res.render('webview', {
        ...
        streamPlatform: req.activeSession?.streamPlatform ?? 'unmanaged',
  ...
  app.post('/api/stream/managed/start', async (req: AuthenticatedRequest, res: any) => {
    try {
      ...
      // Handle unmanaged RTMP streaming
      if (req.activeSession.streamPlatform === 'unmanaged') {
        const DEFAULT_RTMP_URL = process.env.STREAM_URL;
        await (req.activeSession as AppSession).camera.startStream({ rtmpUrl: DEFAULT_RTMP_URL });
        req.activeSession.streamType = 'unmanaged';
        req.activeSession.streamStatus = 'starting';
        req.activeSession.directRtmpUrl = DEFAULT_RTMP_URL;
